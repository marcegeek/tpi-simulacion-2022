...
\section{Paso 1: Definición de sistema bajo estudio}
...
\section{Paso 2: Generación del modelo de simulación base}
...
\section{Paso 3: Recolección y análisis de datos}
    \subsection{Capacidad de caza de un individuo}
...
\section{Paso 4: Generación del modelo preliminar}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics{/images/imagen1}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics{/images/imagen2}
        \caption{Cantidad de yaguaretés hembras en comparación con cantidad de machos}
        \label{fig:fig2}
        \centering
        \includegraphics[width=350pt]{/images/imagen4}
        \caption{Cantidad de individuos en el área por año}
        \label{fig:fig4}
    \end{figure}

\section{Pasos 5 y 6: Verificación y validación del modelo}
    \subsection{Verificación del modelo}
    Hemos comprobado en varias ocasiones el modelo, donde en general han surgido problemas con el movimiento en las áreas o distancias entre los individuos.
    Las distribuciones previamente mencionadas corresponden en general a un informe de confianza donde se estudió el estilo de vida del yaguareté y en la simulación recreada hemos confirmado de que funcionan correctamente con lo que representabamos.
    El modelo luego de ciertos arreglos principalmente del movimiento de los agentes, funcionó correctamente, pudiendo llegar así a los resultados finales.

    \subsection{Validación del modelo}
    Para moldear el modelo de manera que se vea lo más realista posible (con la dificultad que lleva modelar la vida real de un animal), los arreglos para la verificación del modelo fueron sobretodo por las distancias entre individuos.
    Casi siempre se notaba que encuentro entre machos era frecuente por lo que esto llevaba a la muerte de muchos de ellos. Esto en la vida real se puede ver pero no de una manera tan frecuente, en general esta especie busca alejarse de los problemas entre ellos a menos que se encuentren muy cerca uno de otro.
    Por esto es que la distribución de muerte de un macho fue reducida al igual que la distancia que produzca esta función programada en el modelo.
    Otro punto que encontramos que nos producía diferencias entre los números de poblaciones reales junto con los resultados que obteníamos era que en general no se producía el apareamiento de manera efectiva.
    Por esto decidimos aumentar el rango de distancia en el cual los machos podrían encontrar a una hembra y aparearse. Esto se cambió de manera que no afectara demasiado el apego a la realidad (que no se produzca a demasiados kilometros de distancia entre un macho y una hembra).

\section{Paso 8: Determinación de los escenarios para el análisis}
...
\section{Paso 7: Generación del modelo final}
    ...
    \section{Paso 9: Análisis de sensibilidad}
...
\section{Paso 10: Documentación del modelo, sugerencias y conclusiones}
